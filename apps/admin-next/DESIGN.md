---
version: alpha
name: Luna Mandala - Admin
scope: apps/admin-next/
brand-source: docs/brand.md
reference: Cal.com (https://cal.com), spec original en docs/design-references/cal.DESIGN.md
description: |
  Superficie operativa interna (Admin). Sigue el sistema de diseño de Cal.com tal cual:
  canvas blanco, CTAs primarios casi-negros (#111), tipografía Cal Sans + Inter,
  cards suaves de #f5f5f5 con esquinas redondeadas ~12px. Light mode por defecto.
  Modo oscuro queda fuera del scope v1.

  La marca Luna Mandala (docs/brand.md) aparece de forma residual:
  - Logo wordmark en el sidebar/header.
  - El morado #583F8B puede sustituir al primary negro solo en el botón de acción
    primaria por pantalla y en el focus ring si se quiere voz de marca discreta.
    Si no, se mantiene el negro de Cal por default.
  - Lima #CDD650, azul #77A6D8 y morado #583F8B se usan SOLO como colores de chip
    semánticos (success / info / primary-state), reemplazando a brand-accent,
    success, badge-* de Cal cuando se quiera marca; o se mantienen los originales
    de Cal si se prefiere consistencia visual con el sistema de origen.
  - Comfortaa NO se usa en admin. Solo Cal Sans + Inter como define este documento.

  El resto del documento es el design system de Cal.com en su forma original.
  Cualquier divergencia debe documentarse aquí explícitamente antes de implementarse.
